...
type API struct {
	...
	Intake        *repo.Intake
	SavedViews    *repo.SavedViews
	Whiteboards   *repo.Whiteboards
	Sheets        *repo.Spreadsheets
	Clips         *repo.Clips
...
func New(store *db.Store, cfg *config.Config, hub *ws.Hub, metrics *obs.Metrics) *API {
	...
	return &API{
		...
		SavedViews:    repo.NewSavedViews(store),
		Whiteboards:   repo.NewWhiteboards(store),
		Sheets:        repo.NewSpreadsheets(store),
		Clips:         repo.NewClips(store),
	}
}
